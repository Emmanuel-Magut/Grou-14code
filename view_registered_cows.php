...
$records = htmlspecialchars($row[0], ENT_QUOTES); 
 ...
 $query = "SELECT cow_id, serial_no_name, gender, breed_type, year_of_birth, cow_photo, registration_date, cow_status FROM cows ORDER BY registration_date ASC"; // #5
...
//  " AS regdat FROM family_members ORDER BY registration_date ASC";
 ...
 $cow_photo=htmlspecialchars($row['cow_photo'], ENT_QUOTES);
  ...
  $cow_status=htmlspecialchars($row['cow_status'], ENT_QUOTES);

 echo '<tr>
 <td><a href="edit_cow_details.php?id=' . $cow_id .'" class="btn btn-success btn-sm">Edit</a></td>
 <td><a href="delete_cow.php?id=' . $cow_id .'" class="btn btn-danger btn-sm">Delete</a></td>
 <td><span class="badge">' . $cow_id . '</span></td>
 <td>' . $serial_no_name . '</td>
 <td>' . $gender. '</td>
 <td>' . $breed_type . '</td>
 <td>' . $year_of_birth .'</td>
 <td><img src="images/'. $cow_photo . '" height="180"></td>
 <td>' . $registration_date . '</td>
 <td><span class="badge">' . $cow_status . '</span></td>
 ...
 // Error message:
 echo '<p class="text-center">The current cows not be ';
